...
module modfielddump
   ...
   logical :: lsavetimeavg = .false.   !< switch for saving time average instead snapshots
...
contains
   ...
   subroutine initfielddump
      ...
      if (lsavetimeavg .and. .not.lnetcdf) stop "lsavetimeavg needs lnetcdf"
      ...
      if (lnetcdf) then
         ...
         if(lsavetimeavg) call allocatevars
   ...
   subroutine addvarsnetcdf
      ...
      if (lqt) vars(:,:,:,ind_qt) = vars(:,:,:,ind_qt) + qt0(2:i1:ncoarse,2:j1:ncoarse,klow:khigh)
      if (lql) vars(:,:,:,ind_ql) = vars(:,:,:,ind_ql) + ql0(2:i1:ncoarse,2:j1:ncoarse,klow:khigh)
      if (lthl) vars(:,:,:,ind_thl) = vars(:,:,:,ind_thl) + thl0(2:i1:ncoarse,2:j1:ncoarse,klow:khigh)

      ! buoyancy
      if (lbuoy) then
         vars(:,:,:,ind_buoy) = vars(:,:,:,ind_buoy) + thv0h(2:i1:ncoarse,2:j1:ncoarse,klow:khigh)
         do k=klow,khigh
      ...
      do n=1,nsv
         if (lsv(n)) then
            vars(:,:,:,ind_sv(n)) = vars(:,:,:,ind_sv(n)) + sv0(2:i1:ncoarse,2:j1:ncoarse,klow:khigh,n)
   ...
   subroutine writevarsbinary
      use modfields, only : u0,v0,w0,thl0,qt0,ql0,sv0,thv0h,thvh
      use modglobal, only : imax,i1,ih,jmax,j1,jh,k1,cexpnr,ifoutput
      use modmicrodata, only : iqr, imicro, imicro_none
